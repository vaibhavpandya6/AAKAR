"""QA agent worker — consumes tasks from Redis and executes them.

Run this as a standalone process:
    python -m agents.qa_agent.worker

The worker runs in an infinite loop, dequeuing tasks from stream:qa_agent
and invoking the QAAgent to perform testing and validation.
"""
...
AGENT_NAME = "qa-agent-1"
AGENT_STREAM = "stream:qa_agent"
...
class QAWorker:
    """QA agent worker process."""

    def __init__(self) -> None:
        self.agent: Optional[QAAgent] = None
        self.task_queue: Optional[TaskQueue] = None
        self.running = False
        self.log = logger.bind(worker="qa", agent_name=AGENT_NAME)
    ...
    async def initialize(self) -> None:
        ...
        # Create consumer group for QA agent stream
...
_worker_instance: Optional[QAWorker] = None
