"""Database agent worker — consumes tasks from Redis and executes them.

Run this as a standalone process:
    python -m agents.database_agent.worker

The worker runs in an infinite loop, dequeuing tasks from stream:database_agent
and invoking the DatabaseAgent to generate Alembic migrations.
"""
...
AGENT_STREAM = "stream:database_agent"
...
class DatabaseWorker:
    """Database agent worker process."""

    def __init__(self) -> None:
        self.agent: Optional[DatabaseAgent] = None
        self.task_queue: Optional[TaskQueue] = None
        self.running = False
        self.log = logger.bind(worker="database", agent_name=AGENT_NAME)
    ...
    async def initialize(self) -> None:
        ...
        # Create consumer group for database agent stream
...
async def main():
    """Entry point for database agent worker."""
    global _worker_instance
