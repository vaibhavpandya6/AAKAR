"""Frontend agent worker — consumes tasks from Redis and executes them.

Run this as a standalone process:
    python -m agents.frontend_agent.worker

The worker runs in an infinite loop, dequeuing tasks from stream:frontend_agent
and invoking the FrontendAgent to generate React/TypeScript components.
"""
...
AGENT_NAME = "frontend-agent-1"
AGENT_STREAM = "stream:frontend_agent"
...
class FrontendWorker:
    """Frontend agent worker process."""

    def __init__(self) -> None:
        self.agent: Optional[FrontendAgent] = None
        self.task_queue: Optional[TaskQueue] = None
        self.running = False
        self.log = logger.bind(worker="frontend", agent_name=AGENT_NAME)
    ...
    async def initialize(self) -> None:
        ...
        # Create consumer group for frontend agent stream
...
_worker_instance: Optional[FrontendWorker] = None
...
async def main():
    """Entry point for frontend agent worker."""
    global _worker_instance
